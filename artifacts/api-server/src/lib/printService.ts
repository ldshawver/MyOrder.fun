...
// ── Shift Print Jobs ─────────────────────────────────────────────────────────
// Enqueue starting or ending inventory receipts for clock-in/clock-out events.

export async function enqueueShiftPrintJob(opts: {
  type: "shift_start" | "shift_end" | "shift_sales" | "shift_tip_summary" | "shift_commission" | "shift_deposit" | "shift_restock" | "shift_discrepancy" | "shift_sales_report";
  shiftId: number;
  operatorName: string;
  clockedInAt: Date;
  clockedOutAt?: Date;
  ...
  const rawItems = await db
    ...
    .where(eq(shiftInventoryItemsTable.shiftId, opts.shiftId))
    .orderBy(asc(shiftInventoryItemsTable.displayOrder));

  const items = rawItems.map(i => ({
    rowType: i.rowType ?? "item",
    sectionName: i.sectionName,
    itemName: i.itemName,
    unitType: i.unitType ?? "#",
    quantityStart: parseFloat(String(i.quantityStart ?? 0)),
    quantitySold: parseFloat(String(i.quantitySold ?? 0)),
    quantityEnd: i.quantityEnd != null ? parseFloat(String(i.quantityEnd)) : undefined,
    isFlagged: i.isFlagged ?? false,
  }));

  const fmt$ = (n?: number) => `$${(n ?? 0).toFixed(2)}`;
  const fmtDate = (d: Date) => d.toLocaleString([], { dateStyle: "short", timeStyle: "short" });

  let renderedText: string;
  if (opts.type === "shift_start") {
    const blocks = buildInventoryStartBlocks({
      shiftId: opts.shiftId,
      operatorName: opts.operatorName,
      clockedInAt: opts.clockedInAt,
      tenantName: opts.tenantName,
      items,
      logoLines,
      footerMessage,
    });
    renderedText = renderBlocks(blocks, width);
  } else if (opts.type === "shift_end") {
    const blocks = buildInventoryEndBlocks({
      shiftId: opts.shiftId,
      operatorName: opts.operatorName,
      clockedInAt: opts.clockedInAt,
      clockedOutAt: opts.clockedOutAt ?? new Date(),
      tenantName: opts.tenantName,
      items,
    ...
    renderedText = renderBlocks(blocks, width);
  } else if (opts.type === "shift_sales") {
    renderedText = renderBlocks([
      { type: "logo", lines: logoLines },
      { type: "spacer" },
      { type: "center", text: "SHIFT SALES SUMMARY" },
      { type: "divider" },
      { type: "kv", left: "Shift #", right: String(opts.shiftId) },
      { type: "kv", left: "Operator", right: opts.operatorName },
      { type: "kv", left: "Clock-In", right: fmtDate(opts.clockedInAt) },
      { type: "kv", left: "Clock-Out", right: fmtDate(opts.clockedOutAt ?? new Date()) },
      { type: "divider", char: "-" },
      { type: "kv", left: "Orders", right: String(opts.orderCount ?? 0) },
      { type: "kv", left: "Total Revenue", right: fmt$(opts.totalSales) },
      { type: "kv", left: "Cash Sales", right: fmt$(opts.cashSales) },
      { type: "kv", left: "Card Sales", right: fmt$(opts.cardSales) },
      ...(opts.compSales ? [{ type: "kv" as const, left: "Comp Sales", right: fmt$(opts.compSales) }] : []),
      { type: "divider" },
      { type: "center", text: new Date().toLocaleString() },
      { type: "spacer" },
    ], width);
  } else if (opts.type === "shift_commission") {
    renderedText = renderBlocks([
      { type: "logo", lines: logoLines },
      { type: "spacer" },
      { type: "center", text: "CSR COMMISSION SLIP" },
      { type: "divider" },
      { type: "kv", left: "Shift #", right: String(opts.shiftId) },
      { type: "kv", left: "Operator", right: opts.operatorName },
      { type: "divider", char: "-" },
      { type: "kv", left: "Eligible Sales", right: fmt$(opts.eligibleSalesBase) },
      { type: "kv", left: "Commission %", right: `${opts.tipPercent ?? 0}%` },
      { type: "kv", left: "Discrepancy", right: `-${fmt$(opts.differenceAmount)}` },
      { type: "kv", left: "Commission Earned", right: fmt$(opts.tipAmount) },
      { type: "divider" },
      { type: "center", text: new Date().toLocaleString() },
      { type: "spacer" },
    ], width);
  } else if (opts.type === "shift_deposit") {
    renderedText = renderBlocks([
      { type: "logo", lines: logoLines },
      { type: "spacer" },
      { type: "center", text: "DEPOSIT SLIP" },
      { type: "divider" },
      { type: "kv", left: "Shift #", right: String(opts.shiftId) },
      { type: "kv", left: "Operator", right: opts.operatorName },
      { type: "divider", char: "-" },
      { type: "kv", left: "Cash Sales", right: fmt$(opts.cashSales) },
      { type: "kv", left: "Starting Bank", right: fmt$(opts.cashBankStart) },
      { type: "kv", left: "Commission", right: `-${fmt$(opts.tipAmount)}` },
      { type: "kv", left: "Deposit Amount", right: fmt$(opts.depositAmount) },
      { type: "divider" },
      { type: "center", text: new Date().toLocaleString() },
      { type: "spacer" },
    ], width);
  } else if (opts.type === "shift_tip_summary") {
    const eligible = opts.totalSales ?? 0;
    renderedText = renderBlocks([
      { type: "logo", lines: logoLines },
      { type: "spacer" },
      { type: "center", text: "TIP ESTIMATE" },
      { type: "divider" },
      { type: "kv", left: "Shift #", right: String(opts.shiftId) },
      { type: "kv", left: "Operator", right: opts.operatorName },
      { type: "kv", left: "Clock-Out", right: fmtDate(opts.clockedOutAt ?? new Date()) },
      { type: "divider", char: "-" },
      { type: "kv", left: "Total Sales", right: fmt$(eligible) },
      { type: "kv", left: "15% Estimate", right: fmt$(eligible * 0.15) },
      { type: "kv", left: "16% Estimate", right: fmt$(eligible * 0.16) },
      { type: "kv", left: "17% Estimate", right: fmt$(eligible * 0.17) },
      { type: "kv", left: "18% Estimate", right: fmt$(eligible * 0.18) },
      { type: "text", text: "  (Pending supervisor selection)" },
      { type: "divider" },
      { type: "center", text: new Date().toLocaleString() },
      { type: "spacer" },
    ], width);
  } else if (opts.type === "shift_restock") {
    const restockItems = items.filter(i => i.rowType === "item" && (i.quantityEnd ?? 0) > 0);
    renderedText = renderBlocks([
      { type: "logo", lines: logoLines },
      { type: "spacer" },
      { type: "center", text: "RESTOCK LIST" },
      { type: "divider" },
      { type: "kv", left: "Shift #", right: String(opts.shiftId) },
      { type: "kv", left: "Operator", right: opts.operatorName },
      { type: "divider", char: "-" },
      ...restockItems.map(i => ({ type: "kv" as const, left: i.itemName, right: `${i.quantityEnd ?? 0} ${i.unitType}` })),
      ...(restockItems.length === 0 ? [{ type: "center" as const, text: "No items to restock" }] : []),
      { type: "divider" },
      { type: "center", text: new Date().toLocaleString() },
      { type: "spacer" },
    ], width);
  } else if (opts.type === "shift_discrepancy") {
    const flagged = items.filter(i => i.isFlagged);
    renderedText = renderBlocks([
      { type: "logo", lines: logoLines },
      { type: "spacer" },
      { type: "center", text: "DISCREPANCY REPORT" },
      { type: "divider" },
      { type: "kv", left: "Shift #", right: String(opts.shiftId) },
      { type: "kv", left: "Operator", right: opts.operatorName },
      { type: "divider", char: "-" },
      ...flagged.map(i => ({ type: "kv" as const, left: i.itemName, right: `Diff: ${i.quantityEnd ?? 0} vs ${i.quantityStart - i.quantitySold}` })),
      ...(flagged.length === 0 ? [{ type: "center" as const, text: "No discrepancies" }] : []),
      { type: "divider" },
      { type: "center", text: new Date().toLocaleString() },
      { type: "spacer" },
    ], width);
    ...
    renderedText = renderBlocks([
      { type: "logo", lines: logoLines },
      { type: "spacer" },
      { type: "center", text: "SHIFT SALES REPORT" },
      { type: "divider" },
      { type: "kv", left: "Shift #", right: String(opts.shiftId) },
      { type: "kv", left: "Operator", right: opts.operatorName },
      { type: "kv", left: "Clock-In", right: fmtDate(opts.clockedInAt) },
      ...
      { type: "divider", char: "-" },
      { type: "kv", left: "Commission", right: fmt$(opts.tipAmount) },
      { type: "kv", left: "Deposit", right: fmt$(opts.depositAmount) },
      { type: "divider" },
      { type: "center", text: new Date().toLocaleString() },
      { type: "spacer" },
    ], width);
  ...
  const iKey = `shift:${opts.type}:${opts.shiftId}:${receiptPrinter.id}`;
  const existing = await db
    .select()
    .from(printJobsTable)
    .where(eq(printJobsTable.idempotencyKey, iKey))
    .limit(1);
  ...
  const [job] = await db
    ...
    .values({
      ...
      idempotencyKey: iKey,
      renderFormat: "text",
      payloadJson: { shiftId: opts.shiftId, type: opts.type },
      renderedText,
      operatorUserId: operator?.userId ?? null,
    })
    .returning();
