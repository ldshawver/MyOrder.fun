...
describe("PayPal release security structure", () => {
  it("fails legacy Stripe endpoints closed without mock fallback reachability", () => { expect(legacy).toContain("LEGACY_PAYMENT_PROVIDER_DISABLED"); expect(legacy.indexOf("LEGACY_PAYMENT_PROVIDER_DISABLED")).toBeLessThan(legacy.indexOf('router.post("/payments/tokenize"')); });
  ...
  it("staging requires PayPal Sandbox variables and keeps Stripe empty/optional", () => { expect(compose).toContain("PAYPAL_CLIENT_SECRET: ${PAYPAL_CLIENT_SECRET:?"); expect(compose).toContain("STRIPE_SECRET_KEY: ${STRIPE_SECRET_KEY:-}"); expect(compose).not.toContain("PAYPAL_API_BASE_URL"); });
});
