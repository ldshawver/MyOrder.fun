...
describe("POST /api/orders — paymentToken validation", () => {
  it("cash order with paymentToken: null succeeds (201)", async () => {
    mockActor = dbState.users[0]!;
    const res = await supertest(buildApp())
      .post("/api/orders")
      .send({ items: [{ catalogItemId: 1, quantity: 1 }], paymentToken: null });
    expect(res.status).toBe(201);
  });

  it("cash order without paymentToken succeeds (201)", async () => {
    mockActor = dbState.users[0]!;
    const res = await supertest(buildApp())
      .post("/api/orders")
      .send({ items: [{ catalogItemId: 1, quantity: 1 }] });
    expect(res.status).toBe(201);
  });

  it("stripe order without paymentToken is rejected (400)", async () => {
    mockActor = dbState.users[0]!;
    const res = await supertest(buildApp())
      .post("/api/orders")
      .send({
        ...
        checkoutConfirmation: {
          ...
          legalDisclaimerText: "I accept the terms.",
          paymentMethod: "stripe",
        },
      });
    expect(res.status).toBe(400);
    expect(res.body.error).toMatch(/paymentToken/i);
  });

  it("stripe order with paymentToken succeeds (201)", async () => {
    mockActor = dbState.users[0]!;
    const res = await supertest(buildApp())
      .post("/api/orders")
      .send({
        ...
        paymentToken: "pi_test_abc123_secret_xyz",
      });
    expect(res.status).toBe(201);
