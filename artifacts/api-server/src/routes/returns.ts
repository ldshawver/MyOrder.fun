  ...
  const outcome = await db.transaction(async (tx) => {
    ...
    for (const c of computed) {
      ...
      if (c.line.disposition === "RESTOCK") {
        ...
        const valuation = rows<Row>(await tx.execute(sql`SELECT average_unit_cost AS cost FROM inventory_valuation_states WHERE tenant_id = ${tenantId} AND catalog_item_id = ${Number(c.item.catalog_item_id)} LIMIT 1`))[0];
